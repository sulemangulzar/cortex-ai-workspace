...
const features = [
  ['01', 'Capture the brief', 'Collect requirements, uploads, and project conversations in one place.'],
  ['02', 'Plan with agents', 'Move from raw ideas to architecture, implementation notes, QA, and review.'],
  ['03', 'Track the output', 'Watch runs, generated files, logs, and reviewer findings as the work evolves.'],
]
...
function Landing({ onAuth }) {
  ...
  return <main className="min-h-screen overflow-hidden bg-[#080808] text-white selection:bg-white selection:text-black"><div className="pointer-events-none fixed inset-0 z-0 bg-[radial-gradient(circle_at_50%_0%,rgba(255,255,255,0.07),transparent_28rem)]" /><nav className="relative z-10 mx-auto flex max-w-7xl items-center justify-between px-6 py-6 lg:px-10"><Logo href="#top" /><div className="hidden items-center gap-8 font-mono text-xs text-white/50 md:flex"><a className="transition hover:text-white" href="#top">Home</a><a className="transition hover:text-white" href="#product">Product</a><a className="transition hover:text-white" href="#principles">Workflow</a><a className="transition hover:text-white" href="#contact">Contact</a></div><div className="hidden items-center gap-4 font-mono text-xs sm:flex"><button onClick={() => onAuth('login')} className="text-white/55 transition hover:text-white">Sign in</button><button onClick={() => onAuth('signup')} className="rounded-full border border-white/20 px-4 py-2 transition hover:border-white/60 hover:bg-white hover:text-black">Create account <ArrowUpRight /></button></div><button aria-label="Toggle navigation" aria-expanded={menuOpen} onClick={() => setMenuOpen(!menuOpen)} className="rounded-md border border-white/20 p-2 font-mono transition-transform duration-200 active:scale-90 md:hidden">{menuOpen ? 'x' : '='}</button></nav><div className={`relative z-20 mx-6 overflow-hidden rounded-xl border border-white/15 bg-[#101010] font-mono text-sm transition-all duration-300 ease-out md:hidden ${menuOpen ? 'max-h-96 translate-y-0 p-5 opacity-100' : 'pointer-events-none max-h-0 -translate-y-2 border-transparent p-0 opacity-0'}`}><div className="flex flex-col gap-5 text-white/60"><a href="#top" onClick={() => setMenuOpen(false)}>Home</a><a href="#product" onClick={() => setMenuOpen(false)}>Product</a><a href="#principles" onClick={() => setMenuOpen(false)}>Workflow</a><a href="#contact" onClick={() => setMenuOpen(false)}>Contact</a><button className="text-left text-white" onClick={() => { setMenuOpen(false); onAuth('login') }}>Sign in</button><button className="text-left text-white" onClick={() => { setMenuOpen(false); onAuth('signup') }}>Create account</button></div></div><section id="top" className="relative z-10 mx-auto grid max-w-7xl gap-12 px-6 pb-20 pt-20 sm:pb-24 sm:pt-28 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-end lg:px-10 lg:pt-36"><div className="max-w-4xl"><p className="mb-8 font-mono text-[10px] uppercase tracking-[0.25em] text-white/35">AI engineering workspace</p><h1 className="font-mono text-5xl font-medium leading-[1.03] tracking-[-0.08em] sm:text-7xl lg:text-[7rem]">Cortex<br /><span className="text-white/35">Studio</span></h1><p className="mt-8 max-w-2xl font-mono text-sm leading-7 text-white/50 sm:text-base">Turn rough requirements into structured engineering direction with project chat, document uploads, realtime agent runs, generated files, and review output in one focused workspace.</p><div className="mt-10 flex flex-col gap-3 font-mono text-xs sm:flex-row"><button onClick={() => onAuth('signup')} className="group inline-flex items-center justify-center gap-3 rounded-full bg-white px-6 py-3.5 text-black transition hover:bg-white/85">Start a workspace <span className="transition group-hover:translate-x-1">→</span></button><a href="#product" className="inline-flex items-center justify-center gap-3 rounded-full border border-white/15 px-6 py-3.5 text-white/65 transition hover:border-white/50 hover:text-white">Preview product <ArrowUpRight /></a></div><div className="mt-12 grid max-w-2xl grid-cols-3 gap-3 border-t border-white/10 pt-5 font-mono"><div><div className="text-xl text-white">8</div><div className="mt-1 text-[10px] leading-4 text-white/35">agent stages</div></div><div><div className="text-xl text-white">Realtime</div><div className="mt-1 text-[10px] leading-4 text-white/35">run visibility</div></div><div><div className="text-xl text-white">Files</div><div className="mt-1 text-[10px] leading-4 text-white/35">ready to inspect</div></div></div></div><div className="rounded-2xl border border-white/15 bg-[#0d0d0d] p-5 shadow-[0_0_70px_rgba(255,255,255,0.05)]"><div className="flex items-center justify-between border-b border-white/10 pb-4 font-mono text-[10px] text-white/35"><span>live pipeline</span><span>running</span></div><div className="mt-5 space-y-3">{workflow.slice(0, 5).map((item, index) => <div key={item} className="flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 font-mono"><span className="text-xs text-white/75">{item}</span><span className={`h-2 w-2 rounded-full ${index < 3 ? 'bg-emerald-300' : 'bg-white/25'}`} /></div>)}</div></div></section><section id="product" className="relative z-10 mx-auto max-w-7xl px-6 pb-28 lg:px-10"><div className="overflow-hidden rounded-2xl border border-white/15 bg-[#0d0d0d] shadow-[0_0_70px_rgba(255,255,255,0.05)]"><div className="flex items-center justify-between border-b border-white/10 px-5 py-4 font-mono text-[10px] text-white/35"><span>cortex / project / overview</span><span className="hidden sm:block">agent run 03</span></div><div className="grid gap-6 p-6 sm:p-10 lg:grid-cols-[1fr_1.1fr]"><div><p className="font-mono text-[10px] uppercase tracking-[0.25em] text-white/30">Your engineering command center</p><h2 className="mt-3 max-w-lg font-mono text-2xl tracking-tight text-white sm:text-3xl">Requirements, planning, code output, and review stay connected.</h2><p className="mt-5 max-w-md font-mono text-xs leading-6 text-white/40">Cortex Studio keeps each project organized around a conversation, uploaded context, generated implementation files, terminal-style logs, and reviewer findings.</p></div><div className="grid gap-4 sm:grid-cols-2"><div className="rounded-xl border border-white/15 bg-white/4 p-5"><div className="font-mono text-[10px] text-white/35">ACTIVE PROJECT</div><div className="mt-12 font-mono text-lg text-white">Launch workflow</div><div className="mt-3 h-1 overflow-hidden rounded-full bg-white/10"><div className="h-full w-3/4 rounded-full bg-white" /></div></div><div className="rounded-xl border border-white/10 bg-white/2 p-5"><div className="font-mono text-[10px] text-white/35">LATEST OUTPUT</div><div className="mt-12 font-mono text-lg text-white/80">Architecture reviewed</div><div className="mt-3 font-mono text-[10px] leading-5 text-white/35">QA, security, performance, and coverage checks queued.</div></div></div></div></div></section><section id="principles" className="relative z-10 mx-auto max-w-7xl border-t border-white/10 px-6 py-24 lg:px-10"><div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr]"><div><p className="font-mono text-[10px] uppercase tracking-[0.25em] text-white/35">/ workflow</p><h2 className="mt-5 max-w-md font-mono text-3xl leading-tight tracking-[-0.06em] sm:text-5xl">From idea<br /><span className="text-white/35">to build-ready.</span></h2></div><div className="grid gap-4 sm:grid-cols-3">{features.map(([number, title, description]) => <article key={number} className="border-t border-white/20 pt-4 transition duration-500 hover:-translate-y-1 hover:border-white/60"><div className="font-mono text-xs text-white/30">{number}</div><h3 className="mt-10 font-mono text-sm">{title}</h3><p className="mt-4 font-mono text-xs leading-6 text-white/40">{description}</p></article>)}</div></div></section><section id="contact" className="relative z-10 mx-auto max-w-7xl border-t border-white/10 px-6 py-24 lg:px-10"><div className="grid gap-10 rounded-2xl border border-white/15 bg-white/3 p-7 sm:p-10 lg:grid-cols-[1fr_auto] lg:items-end"><div><p className="font-mono text-[10px] uppercase tracking-[0.25em] text-white/35">/ contact</p><h2 className="mt-5 max-w-xl font-mono text-3xl leading-tight tracking-[-0.06em] sm:text-5xl">Build with a clearer<br /><span className="text-white/35">starting point.</span></h2><p className="mt-5 max-w-md font-mono text-xs leading-6 text-white/40">Share the product brief, upload the requirements, and let the workspace turn early ambiguity into structured engineering direction.</p></div><button onClick={() => onAuth('signup')} className="inline-flex w-fit items-center gap-3 rounded-full bg-white px-6 py-3.5 font-mono text-xs text-black transition hover:bg-white/85">Create workspace <ArrowUpRight /></button></div></section><footer className="relative z-10 mx-auto flex max-w-7xl flex-col gap-6 border-t border-white/10 px-6 py-8 font-mono text-[10px] text-white/30 sm:flex-row sm:items-center sm:justify-between lg:px-10"><span>© 2026 cortex.ai</span><span>AI engineering workspace<span className="text-white">_</span></span><a href="#top" className="text-white/60 transition hover:text-white">back to top</a></footer></main>
}
